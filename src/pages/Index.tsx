...
import { VideoForgeTab } from '@/components/VideoForge/VideoForgeTab';
import { useLocalStorage } from '@/hooks/useLocalStorage';
import { useVoiceGeneration } from '@/hooks/useVoiceGeneration';
import { VoiceId, GenerationSettings } from '@/types';
import { buildStyleInstructions } from '@/config/styleMixer';
...
export default function Index() {
  ...
  const [styleInstructions, setStyleInstructions] = useState(
    buildStyleInstructions(['warm', 'conversational', 'dramatic'])
  );
  ...
  const handleStyleChange = useCallback((instructions: string, recommendedVoice?: VoiceId) => {
  ...
  }, [apiKey, script, selectedVoice, styleInstructions, generate, reset]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
            ...
            <div className="space-y-6">
              <div className="lg:sticky lg:top-16">
                <div className="rounded-xl border border-border bg-card p-5 space-y-6">
                  ...
                  <StyleMixer onStyleChange={handleStyleChange} />
      ...
      {/* VideoForge Tab - full width, no container padding */}
      {activeTab === 'video' && (
        <main className="flex-1 flex flex-col overflow-hidden">
          <VideoForgeTab
            voiceoverUrl={audioUrl ?? undefined}
            voiceoverBlob={audioBlob ?? undefined}
            script={script}
          />
        </main>
      )}
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-video/5 rounded-full blur-3xl" />
      </div>
    </div>
  );
}
